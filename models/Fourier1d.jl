mutable struct Fourier1d <: SuperRes #1D Static model
    freqs :: Vector{Int}
...
function setBounds(model :: Fourier1d, bounds)
    model.bounds = bounds
    model.approx_grid = range(bounds[1][1], bounds[2][1], length=length(model.approx_grid))
end

function psi(model :: Fourier1d, theta :: Vector{Float64})
    # This function computes the direct problem for a single point theta
    psi_cpx =vec(ComplexF64[model.filter[k] * exp(-2im * pi * model.freqs[k] / model.x_max* theta[1])                                   for k in eachindex(model.freqs)])
...
function dpsi(model :: Fourier1d, theta :: Vector{Float64})
    ...
    dx = vec(ComplexF64[-model.filter[k] * model.freqs[k]/model.x_max* 2im * pi *
...
mutable struct DynamicFourier1d <: DynamicSuperRes #1D Space-time model
    static :: Fourier1d
...
function setBounds(model :: DynamicFourier1d, bounds)
    ...
    model.bounds = bounds
    model.grid_v = range(bounds[1][2], bounds[2][2], length=length(grid_v))
end
